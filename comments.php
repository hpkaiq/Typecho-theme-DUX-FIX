...
$submit_ = $_POST['submit'];
$file_ = $_POST['file'];
$name_ = $_POST['name'];
$type_ = $_POST['type'];
$c_encode = $_GET['text'];
$dl_ = base64_decode($file_);
if(strstr($dl_, 'api', TRUE)){ 
  $file_url = '<font color="blue">评论文件</font>:'.'<a href="'.'https://pan.cccyun.cc/'.strstr($dl_, 'down').'"><font color="red">'.$name_.'</font></a>';
}
else if($file_){
  $file_url = '<font color="blue">评论文件</font>:'.'<a href="'.base64_decode($file_).'"><font color="red">'.$name_.'</font></a>';
}
if($c_encode && $file_url){
...
<?php $this->comments()->to($comments); ?>
  <?php if($this->commentsNum>0 || $this->allow('comment')): ?>
...
<?php if($this->user->hasLogin()): ?>
                   ...
                   <?php elseif($this->remember('author',true) != "" && $this->remember('mail',true) != ""): ?><?php $this->remember('author'); ?><?php endif; ?>
                    ...
                    <textarea placeholder="欢迎在此评论， be nice & cool." class="input-block-level comt-area" name="text" id="comment" cols="100%" rows="3" tabindex="1" ><?php echo $file_url; ?></textarea>
...
<script type="text/javascript" src="https://hpk.me/js_own/jquery.base64.js"></script>
<script>
function beforeSubmit(){
    var f = document.getElementById("tf");
    var nurl = window.location.href.split("#")[0];
    var url = nurl.split("?")[0]+ "?text=" + encodeURIComponent(document.getElementById("comment").value);
    f.action = f.action + $.base64.encode(url) ;  
    return true;
    //f.submit();          
}
